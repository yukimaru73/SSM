...
Quaternion = {
	...
	---@section newFromVector3
	---@param cls Quaternion
	---@param vector Vector3
	---@return Quaternion
	newFromVector3 = function(cls, vector)
		return cls:_new(vector[1], vector[2], vector[3], 0)
	end,
	---@endsection
	...
	add = function(self, target)
		return self:_new(self.x + target.x, self.y + target.y, self.z + target.z, self.w + target.w)
	end,
	---@endsection
	...
	mul = function(self, scalar)
		return self:_new(self.x * scalar, self.y * scalar, self.z * scalar, self.w * scalar)
	end,
	---@endsection
	...
	---@param z number
	---@return Quaternion
	newFromEuler = function(self, x, y, z)
	...
	---@param self Quaternion
	---@param n number
	---@overload fun(self:Quaternion):Quaternion
	---@return Quaternion
	normalize = function(self, n)
		n = math.sqrt(self.x ^ 2 + self.y ^ 2 + self.z ^ 2 + self.w ^ 2)
		return self:_new(self.x / n, self.y / n, self.z / n, self.w / n)
	end,
	---@endsection
	...
	---@param q Quaternion
	---@overload fun(self:Quaternion, vector:Vector3):Vector3
	---@return Vector3
	rotateVector = function(self, vector, q)
		q = self:product(self:_new(vector[1], vector[2], vector[3], 0):product(self:getConjugateQuaternion()))
